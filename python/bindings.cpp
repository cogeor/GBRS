...
// Helper function to build quantile map from user-provided or auto-generated quantiles
std::unordered_map<int, Eigen::VectorXd>
build_quantile_map(const Eigen::MatrixXd& m,
                   int n_quantiles,
                   py::object user_quantiles)
{
    ...
    if (!user_quantiles.is_none()) {
        ...
        if (qlist.size() != static_cast<size_t>(n_features)) {
            throw std::runtime_error("Length of user_quantiles list must match number of features (columns in X)");
        }
