...
def run_benchmarks():
    ...
    sizes = [
        ...
        (100000, 10, 10),    # 100K samples, 10 features, 10 iterations (fewer iters for speed)
    ]
    ...
    for n_samples, n_features, n_iter in sizes:
        print(f"\n{'─' * 70}")
        print(f"Dataset: {n_samples:,} samples × {n_features} features ({n_iter} iterations)")
        print(f"{'─' * 70}")
        ...
        # Survival
        print(f"  Survival...", end=" ", flush=True)
    ...
    for i, (n_samples, n_features, n_iter) in enumerate(sizes):
        ...
        print(f"{size_str:<12} {'Regression':<15} {reg['time_seconds']:<12.3f} MSE: {reg['mse']:.4f}")
        
        clf = results['classification'][i]
        print(f"{size_str:<12} {'Classification':<15} {clf['time_seconds']:<12.3f} Acc: {clf['accuracy']:.2%}")
        
        surv = results['survival'][i]
        print(f"{size_str:<12} {'Survival':<15} {surv['time_seconds']:<12.3f} Range: {surv['risk_range']:.2f}")
        print()
