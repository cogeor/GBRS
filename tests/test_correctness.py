...
def test_regression_vs_linear_baseline(diabetes_data, linear_baseline):
    ...
    assert gbrs_mse < 2.0 * baseline_mse, \
    ...
    mean_pred_mse = calculate_mse(diabetes_data['y_test'], 
                                   np.full_like(diabetes_data['y_test'], 
                                               diabetes_data['y_train'].mean()))
    assert gbrs_mse < mean_pred_mse, \
...
def test_classification_vs_logistic_baseline(breast_cancer_data, logistic_baseline):
    ...
    assert abs(gbrs_acc - baseline_acc) < 0.05, \
        f"GBRS accuracy ({gbrs_acc:.2%}) differs from baseline ({baseline_acc:.2%}) by more than 5%"
    
    # GBRS should be better than random guessing
    majority_class_acc = np.max(np.bincount(breast_cancer_data['y_train'])) / len(breast_cancer_data['y_train'])
    assert gbrs_acc > majority_class_acc, \
...
def test_survival_vs_cox_baseline(veteran_data, cox_baseline):
    ...
    gbrs_cindex = calculate_c_index(
        veteran_data['time_test'],
        veteran_data['event_test'],
        gbrs_risk
    )
