...
std::unordered_map<int, VectorXd> make_quantiles(const MatrixXd &x,
                                                 const int n_pts);
...
private:
  ...
  void iter_proba(const MatrixXd &x, const VectorXd &res, const VectorXd &y,
                  const std::unordered_map<int, VectorXd> &qtsw);
  void iter_survival(const MatrixXd &x, const VectorXd &T, const VectorXd &E,
                     const std::unordered_map<int, VectorXd> &qtsw,
                     const VectorXd &f);
  std::array<double, 4> get_best_split(const VectorXd &u, const VectorXd &y,
                                       const VectorXd &interp_pts);
  std::array<double, 4> get_best_split_proba(const VectorXd &u,
                                             const VectorXd &preds,
                                             const VectorXd &y,
                                             const VectorXd &interp_pts);
  std::array<double, 4>
  get_best_split_survival(const VectorXd &u, const VectorXd &gradients,
  ...
  MatrixXd slice_matrix(const MatrixXd &m, const std::vector<int> &idxs) const {
    if (idxs.empty())
      return m; // Should not happen with current logic but safe check
...
public:
  ...
  void fit_proba(const MatrixXd &m, const VectorXd &y,
                 const std::unordered_map<int, VectorXd> &qtsw);
  void fit_survival(const MatrixXd &X, const VectorXd &T, const VectorXd &E,
  ...
  void prune();
  void set_params(const VectorXd &idxs, const VectorXd &split_val,
                  const VectorXd &w, const double y0);
};
...
VectorXd quantiles(const VectorXd &v, const int n_quantiles) {
  ...
  std::vector<double> sorted(
      v.data(), v.data() + v.size()); // need to make a copy to sort
  int n_samples = v.size();
  std::sort(sorted.begin(), sorted.end());

  for (int i = 0; i < n_quantiles; i++) {
    int idx = static_cast<int>(static_cast<double>(i + 1) / (n_quantiles + 1) *
                               (n_samples - 1));
    out[i] = sorted[idx];
  }
  return out;
...
std::array<double, 2> gamma_survival(const VectorXd &grps,
...
double ranking_loss(const VectorXd &f, const VectorXd &T, const VectorXd &E,
                    const VectorXd &mask) {
  const auto n = f.size();
  double loss = 0.0;
  int pair_count = 0;
...
VectorXd compute_ranking_gradients_par(const VectorXd &f, const VectorXd &T,
                                       const VectorXd &E,
                                       const VectorXd &mask) {
  const auto n = f.size();
...
VectorXd compute_ranking_gradients(const VectorXd &f, const VectorXd &T,
...
std::array<double, 4> Model::get_best_split(const VectorXd &u,
                                            const VectorXd &y,
                                            const VectorXd &interp_pts) {
...
std::array<double, 4> Model::get_best_split_proba(const VectorXd &u,
                                                  const VectorXd &preds,
                                                  const VectorXd &y,
                                                  const VectorXd &interp_pts) {
...
std::array<double, 4>
Model::get_best_split_survival(const VectorXd &u, const VectorXd &gradients,
                               const VectorXd &f, const VectorXd &T,
                               const VectorXd &E, const VectorXd &interp_pts) {
...
const VectorXd &Model::get_idxs() const { return this->idxs; }

const VectorXd &Model::get_split_val() const { return this->split_val; }
...
VectorXd compute_feature_bin_means(const VectorXd &x_col,
                                   const VectorXd &splits) {
...
std::unordered_map<int, VectorXd> compute_binned_feature_means(
    const MatrixXd &X, const std::unordered_map<int, VectorXd> &split_map) {
...
void Model::iter(const MatrixXd &x, const VectorXd &y,
                 const std::unordered_map<int, VectorXd> &qtsw) {
...
#pragma omp parallel for shared(x_ptr, y_ptr, qtsw) schedule(dynamic)
  for (int i = 0; i < x_ptr->rows(); i++) {
    std::array<double, 4> out_tmp =
        this->get_best_split(x_ptr->row(i), *y_ptr, qtsw.at(i));
  ...
  this->add_elem(min_idx, iter_out(min_idx, 1), lr * iter_out(min_idx, 2),
                 lr * iter_out(min_idx, 3));
}

void Model::iter_proba(const MatrixXd &x, const VectorXd &preds,
                       const VectorXd &y,
                       const std::unordered_map<int, VectorXd> &qtsw) {
  std::vector<int> indices = get_batch_indices(y.size());
  MatrixXd x_sub;
...
#pragma omp parallel for shared(x_ptr, preds_ptr, y_ptr, qtsw) schedule(dynamic)
  for (int i = 0; i < x_ptr->rows(); i++) {
    std::array<double, 4> out_tmp =
        get_best_split_proba(x_ptr->row(i), *preds_ptr, *y_ptr, qtsw.at(i));
...
void Model::iter_survival(const Eigen::MatrixXd &x, const Eigen::VectorXd &T,
...
#pragma omp parallel for shared(x_ptr, gradients, f_ptr, T_ptr, E_ptr,         \
  ...
  for (Eigen::Index i = 0; i < x_ptr->rows(); ++i) {
    ...
    std::array<double, 4> out_tmp = this->get_best_split_survival(
        ...
        gradients, *f_ptr, *T_ptr, *E_ptr, qtsw.at(i));
  ...
  this->add_elem(min_idx, iter_out(min_idx, 1), // split point
                 lr * iter_out(min_idx, 2),     // left update
                 lr * iter_out(min_idx, 3));    // right update
}

void Model::fit_proba(const MatrixXd &m, const VectorXd &y,
                      const std::unordered_map<int, VectorXd> &qts) {
  VectorXd res(y.size());
  res.setConstant(0.0);
...
void Model::fit(const MatrixXd &m, const VectorXd &y,
                const std::unordered_map<int, VectorXd> &qts) {
  VectorXd res = y;
  VectorXd out_model(y.size());
  for (int i = 0; i < this->max_n; i++) {
...
void Model::fit_survival(const MatrixXd &m, const VectorXd &T,
                         const VectorXd &E,
                         const std::unordered_map<int, VectorXd> &qts) {
...
void Model::set_params(const VectorXd &idxs, const VectorXd &split_val,
  ...
  // Zero out w1/w2 as we don't have them from JSON usually, and they are used
  // for training mostly? Or we can set them to 0.
  this->w1.setZero();
  this->w2.setZero();
}

const ScoreParams &Model::get_params() const { return this->params; }
...
void Model::prune() {
  ...
  for (int j = 0; j < this->i; j++) {
    ...
    if (merged.find(key) != merged.end()) {
      ...
      merged[key] =
          std::make_tuple(this->params.w[j], this->w1[j], this->w2[j]);
    }
  }
